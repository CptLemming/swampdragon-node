var DataMapper = function (channelMaps) {
  ...
  dataMapper.mapData = function(datasource, message) {
    if (message.action == "created") {
      return dataMapper.mapCreated(datasource, message.data);
    }
    else if (message.action == "updated") {
      return dataMapper.mapUpdated(datasource, message.data);
    }
    else if (message.action == "deleted") {
      return dataMapper.mapDelete(datasource, message.data);
    }
  };
  ...
  dataMapper.mapUpdated = function(datasource, data) {
    ...
    if (instances.length === 0 && !channelMap) {
      if (datasource instanceof Array) {
        datasource.push(data);
      } else {
        datasource = data;
    ...
    if (parents.length === 0) {
      if (datasource instanceof Array) {
        for (var j in datasource) {
          if (dataMapper._compareIds(datasource[j].id, data.id)) {
  ...
  dataMapper._findCurrentParents = function(datasource, data, channelMap, result) {
    result = result || [];
    channelMap = channelMap || dataMapper._findMapping(data._type);
    if (!channelMap) {
      return result;
    }
